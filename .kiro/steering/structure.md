...
ソリューション/プロジェクト単位で機能を分割し、UI・アプリケーションサービス・ドメイン・インフラを層分離。競技課題ごとにサブプロジェクトを追加するが、共通基盤（DBスクリプト、ユーティリティ）は再利用する。

## Directory Patterns

### ソリューション/アプリ本体
**Location**: `/`  
**Purpose**: Visual Studio ソリューションと .NET デスクトップアプリのプロジェクトを配置。UI層はWinForms/WPFに応じた`Views`/`Controls`配下、プレゼンターやViewModelは別フォルダで管理。  
...
**Purpose**: ビジネスルールとユースケースをUIから分離。ドメインモデル、サービス、DTOを配置。  
**Example**: `Order.cs`, `OrderService.cs`, `OrderDto.cs`
...
- **Components**: クラスはPascalCase、インターフェースは`I`接頭辞。フォーム/ウィンドウは`*Form`/`*Window`。
